...
export default function ProfessorPostDetailPage() {
  ...
  const fetchPost = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/v1/professor/community/posts/${postId}`,
        {
  ...
  const handleVote = async (targetId: number, voteType: 'upvote' | 'downvote') => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:8080/api/v1/professor/community/vote/${targetId}`,
        {
          userType: 'professor',
          targetType: 'post',
          voteType
  ...
  const handleCommentSubmit = async (content: string, parentCommentId?: number) => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:8080/api/v1/professor/community/posts/${postId}/comments`,
        {
          content,
      ...
      <div className="min-h-screen bg-gradient-to-b from-purple-100 via-orange-50 to-transparent p-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-6">
            ...
            <button
              onClick={() => router.push('/professor/community')}
