...
export default function ProfessorCommunityPage() {
  ...
  const initializeTables = async () => {
    try {
      ...
      await axios.get('http://localhost:8080/api/v1/professor/community/init', {
  ...
  const fetchClasses = async () => {
    try {
      ...
      if (response.data.success) {
        const professorClasses = response.data.data.map((cls: any) => ({
          id: cls.id,
          name: cls.name
        }));
        ...
        setError('No classes assigned to professor');
        setLoading(false);
      }
    } catch (error: any) {
  ...
  const fetchPosts = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/v1/professor/community/posts?${params}`, 
        {
          headers: { Authorization: `Bearer ${token}` },
          withCredentials: true,
        }
  ...
  const fetchUserVotes = async (postIds: number[]) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/v1/professor/community/votes?${params}`,
        {
          headers: { Authorization: `Bearer ${token}` },
          withCredentials: true,
        }
  ...
  const handleCreatePost = async (postData: { title: string; content: string; classId: number }) => {
    try {
      ...
      const response = await axios.post(
        'http://localhost:8080/api/v1/professor/community/posts',
        {
          ...postData,
  ...
  const handleVote = async (postId: number, voteType: 'upvote' | 'downvote') => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:8080/api/v1/professor/community/vote/${postId}`,
        {
          userType: 'professor',
          targetType: 'post',
          voteType
  ...
  const handlePostClick = (postId: number) => {
    router.push(`/professor/community/post/${postId}`);
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">Engage with your students and colleagues</p>
          </div>
          <button
